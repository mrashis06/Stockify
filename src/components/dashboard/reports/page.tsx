...
export default function ReportsPage({ params, searchParams }: { params: { slug: string }; searchParams?: { [key: string]: string | string[] | undefined } }) {
    ...
    const handleDateSelect = (range: DateRange | undefined) => {
        if (range?.from) {
            if (!range.to) {
                // If only a start date is selected, treat it as a single-day selection
                setDate({ from: range.from, to: range.from });
            } else if (isSameDay(range.from, range.to)) {
                // If the user clicks the same date twice, confirm single-day
                setDate({ from: range.from, to: range.from });
            } else {
